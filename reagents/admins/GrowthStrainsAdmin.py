...
class GrowthStrainsAdminAbstract(admin.ModelAdmin):

	list_display = ('growthstrain_id','growthstrain_name',)
	
	search_fields = ['growthstrain_id','growthstrain_name',]
	readonly_fields = ('growthstrain_id',)

	fieldsets = [
		('',{
			'classes': ('suit-tab suit-tab-growthstrain',),
			'fields': ['growthstrain_name']
		}),
	]
	suit_form_tabs = [
		(u'growthstrain', u'Growth strain')
	]
	...
	class Media:
		...
		js = ('generic.js','models/growthstrains.js')

	

	def get_actions(self, request):
		actions = super(GrowthStrainsAdminAbstract, self).get_actions(request)
	...
	def construct_change_message(self, request, form, formsets):
		message = super(GrowthStrainsAdminAbstract, self).construct_change_message(request, form, formsets)
		change_message = []
